...
package_version="$(awk '$1 == "version:" { print $2; exit }' "$repo_dir/native/package/manifest.yml")"
...
mkdir -p "$stage_dir/net48" "$stage_dir/net7.0" "$output_dir"
...
cp "$repo_dir/native/RhinoMCP.Plugin/bin/$configuration/net48/RhinoMCP.rhp" "$stage_dir/net48/"
cp "$repo_dir/native/RhinoMCP.Grasshopper/bin/$configuration/net48/RhinoMCP.Grasshopper.gha" "$stage_dir/net48/"
cp "$repo_dir/native/RhinoMCP.Plugin/bin/$configuration/net48/"*.dll "$stage_dir/net48/"
cp "$repo_dir/native/package/manifest.yml" "$stage_dir/"

if command -v yak >/dev/null 2>&1; then
  (cd "$stage_dir" && yak build --platform win)
  mv "$stage_dir"/*.yak "$output_dir/"
else
  package="$output_dir/rhino-mcp-easy-$package_version-rh8_0-win.yak"
